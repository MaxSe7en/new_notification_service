...
<?php
// File: /app/Workers/WebSocketServer.php
// namespace App\Workers;

// use OpenSwoole\WebSocket\Server; // Changed from Swoole\WebSocket\Server
// use OpenSwoole\Timer;            // Changed from Swoole\Timer
...
//             'ssl_cert_file' => '/etc/letsencrypt/live/[winsstarts.com/fullchain.pem](https://winsstarts.com/fullchain.pem)',
//             'ssl_key_file' => '/etc/letsencrypt/live/[winsstarts.com/privkey.pem](https://winsstarts.com/privkey.pem)',
...
//         // $this->verifyKeyTypes(); // This method seems to use hardcoded keys 'ws_connections' and 'ws_connection_map'
//                                   // which are not consistent with RedisService2 constants.
//                                   // It's better to rely on RedisService2::USER_CONNECTION_MAP and RedisService2::FD_USER_MAP
//                                   // for consistency. I've commented it out as it might cause issues.
//     }
...
//                 break;
//             case 'send_notification':
//                 // This action should typically be triggered by an internal service, not directly by a client.
//                 // If a client is requesting to send a notification, ensure proper authorization.
//                 $this->sendDirectNotification(
//                     $data['user_id'],
//                     $data['message'] ?? '',
//                     $data['event'] ?? 'notification'
...
//             $userIds = $client->hkeys('ws_connections'); // This uses 'ws_connections' key, ensure it's consistent
//             if (!is_array($userIds)) {
//                 $userIds = [];
//             }
//             foreach ($userIds as $userId) {
//                 $fd = $client->hget('ws_connections', $userId); // This uses 'ws_connections' key
//                 if ($fd && $client->exists("ws_connection:{$fd}")) { // This uses 'ws_connection:{fd}' key
...
//     // This method uses hardcoded Redis keys and might conflict with RedisService2 constants.
//     // It's recommended to remove it or update it to use the constants for consistency.
//     // For now, it's commented out in the constructor.
//     /*
